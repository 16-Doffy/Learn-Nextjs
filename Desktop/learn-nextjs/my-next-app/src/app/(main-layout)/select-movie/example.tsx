...
const CinemaSchedule = () => {
  ...
  // Mock data for showtimes (in a real app, this would come from API)
  const movieShowtimes: Record<number, string[]> = {
    1: ['10:00', '13:00', '14:55', '16:00', '18:20', '19:40', '22:00'],
    2: ['10:45', '13:15', '15:30', '17:45', '19:00', '20:00', '21:15', '22:15', '23:20'],
    3: ['13:35', '15:50', '17:55']
  };
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        // In a real app, you would fetch from your cinema API
        const res = await axios.get(
          'https://api.themoviedb.org/3/movie/now_playing?api_key=733d08f3b55d5c3b516692a4f30a1ff7&language=vi-VN&region=VN'
        );
        
        // Add mock runtime, genres and certification for demo
        const moviesWithDetails = res.data.results.slice(0, 3).map((movie: any, index: number) => ({
          ...movie,
          runtime: [126, 119, 105][index % 3], // Mock runtime in minutes
          genres: [
            { id: 1, name: 'Action' },
            { id: 2, name: 'Adventure' },
            { id: 3, name: 'Animation' }
          ].slice(0, 2 + (index % 2)),
          certification: ['P', 'K', 'T13'][index % 3] // Mock certification
        }));
        
        setMovies(moviesWithDetails);
        setSelectedDate(availableDates[0]); // Set default to today
      } catch (error) {
  ...
  const handleBooking = () => {
    ...
    if (!selectedTime) return alert('Vui lòng chọn suất chiếu');
